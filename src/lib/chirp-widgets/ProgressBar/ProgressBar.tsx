...
export const ProgressBar: React.FC<Props> = ({ current, min, max, avg, postfix, gradientColors, currentColor }) => {
    ...
    return (
        ...
        <CalculatedValue value={max} postfix={postfix} label='max' color={palette.alerts.alert} /> */}
                <CalculatedValue value={min} postfix={postfix} label="min" color="red" />
                <CalculatedValue value={avg} postfix={postfix} label="avg" color="green" />
                <CalculatedValue value={max} postfix={postfix} label="max" color="blue" />
            </Stack>
            <S.Slider
                ref={sliderRef}
                value={current}

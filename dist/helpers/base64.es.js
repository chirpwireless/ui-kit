...
for (var k = 0, _r = H.length; k < _r; ++k)
  S[k] = H[k], R[H.charCodeAt(k)] = k;
R[45] = 62;
R[95] = 63;
function hr(f) {
  var c = f.length;
...
function Mr(f) {
  var c, l = hr(f), x = l[0], y = l[1], a = new Sr(Nr(f, x, y)), p = 0, o = y > 0 ? x - 4 : x, B;
  for (B = 0; B < o; B += 4)
    c = R[f.charCodeAt(B)] << 18 | R[f.charCodeAt(B + 1)] << 12 | R[f.charCodeAt(B + 2)] << 6 | R[f.charCodeAt(B + 3)], a[p++] = c >> 16 & 255, a[p++] = c >> 8 & 255, a[p++] = c & 255;
  return y === 2 && (c = R[f.charCodeAt(B)] << 2 | R[f.charCodeAt(B + 1)] >> 4, a[p++] = c & 255), y === 1 && (c = R[f.charCodeAt(B)] << 10 | R[f.charCodeAt(B + 1)] << 4 | R[f.charCodeAt(B + 2)] >> 2, a[p++] = c >> 8 & 255, a[p++] = c & 255), a;
}
function br(f) {
  return S[f >> 18 & 63] + S[f >> 12 & 63] + S[f >> 6 & 63] + S[f & 63];
}
function kr(f, c, l) {
  for (var x, y = [], a = c; a < l; a += 3)
    x = (f[a] << 16 & 16711680) + (f[a + 1] << 8 & 65280) + (f[a + 2] & 255), y.push(br(x));
  return y.join("");
}
function Dr(f) {
  for (var c, l = f.length, x = l % 3, y = [], a = 16383, p = 0, o = l - x; p < o; p += a)
    y.push(kr(f, p, p + a > o ? o : p + a));
  return x === 1 ? (c = f[l - 1], y.push(
    S[c >> 2] + S[c << 4 & 63] + "=="
  )) : x === 2 && (c = (f[l - 2] << 8) + f[l - 1], y.push(
    S[c >> 10] + S[c >> 4 & 63] + S[c << 2 & 63] + "="
  )), y.join("");
}
var X = {};
/*! ieee754. BSD-3-Clause License. Feross Aboukhadijeh <https://feross.org/opensource> */
X.read = function(f, c, l, x, y) {
  var a, p, o = y * 8 - x - 1, B = (1 << o) - 1, T = B >> 1, I = -7, F = l ? y - 1 : 0, _ = l ? -1 : 1, A = f[c + F];
  for (F += _, a = A & (1 << -I) - 1, A >>= -I, I += o; I > 0; a = a * 256 + f[c + F], F += _, I -= 8)
    ;
  for (p = a & (1 << -I) - 1, a >>= -I, I += x; I > 0; p = p * 256 + f[c + F], F += _, I -= 8)
    ;
  if (a === 0)
    a = 1 - T;
  else {
...
X.write = function(f, c, l, x, y, a) {
  var p, o, B, T = a * 8 - y - 1, I = (1 << T) - 1, F = I >> 1, _ = y === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0, A = x ? 0 : a - 1, D = x ? 1 : -1, P = c < 0 || c === 0 && 1 / c < 0 ? 1 : 0;
  for (c = Math.abs(c), isNaN(c) || c === 1 / 0 ? (o = isNaN(c) ? 1 : 0, p = I) : (p = Math.floor(Math.log(c) / Math.LN2), c * (B = Math.pow(2, -p)) < 1 && (p--, B *= 2), p + F >= 1 ? c += _ / B : c += _ * Math.pow(2, 1 - F), c * B >= 2 && (p++, B /= 2), p + F >= I ? (o = 0, p = I) : p + F >= 1 ? (o = (c * B - 1) * Math.pow(2, y), p = p + F) : (o = c * Math.pow(2, F - 1) * Math.pow(2, y), p = 0)); y >= 8; f[l + A] = o & 255, A += D, o /= 256, y -= 8)
    ;
  for (p = p << y | o, T += y; T > 0; f[l + A] = p & 255, A += D, p /= 256, T -= 8)
    ;
  f[l + A - D] |= P * 128;
};
/*!
 ...
 */
(function(f) {
  const c = G, l = X, x = typeof Symbol == "function" && typeof Symbol.for == "function" ? Symbol.for("nodejs.util.inspect.custom") : null;
  f.Buffer = o, f.SlowBuffer = cr, f.INSPECT_MAX_BYTES = 50;
  const y = 2147483647;
  f.kMaxLength = y, o.TYPED_ARRAY_SUPPORT = a(), !o.TYPED_ARRAY_SUPPORT && typeof console < "u" && typeof console.error == "function" && console.error(
  ...
  function A(i) {
    const r = i.length < 0 ? 0 : Y(i.length) | 0, t = p(r);
    for (let n = 0; n < r; n += 1)
      t[n] = i[n] & 255;
    return t;
  }
  ...
  function Ir(i, r, t) {
    b(r, "offset"), (i[r] === void 0 || i[r + t] === void 0) && $(r, i.length - (t + 1));
  }
  ...
  function $(i, r, t) {
    throw Math.floor(i) !== i ? (b(i, t), new M.ERR_OUT_OF_RANGE("offset", "an integer", i)) : r < 0 ? new M.ERR_BUFFER_OUT_OF_BOUNDS() : new M.ERR_OUT_OF_RANGE(
      "offset",
      `>= 0 and <= ${r}`,
      i
    );
...
function $r(f) {
  return f ? V.Buffer.from(f, "base64").toString("utf-8") : "";
}
function Or(f) {
  return f ? V.Buffer.from(f, "utf-8").toString("base64") : "";
}
export {
  $r as decodeBase64,
  Or as encodeBase64
